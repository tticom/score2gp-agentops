...
def test_reconciliation_end_to_end_produces_clean_governance_audit(tmp_path, monkeypatch, capsys) -> None:
    from scripts.score2gp_orca_control import reconcile_post_merge
    from scripts.score2gp_orchestrator import render_active_task
    ...
    facts = {
        ...
        "pull_request": {
            "number": 639,
            "state": "MERGED",
            "head_branch": "codex/orc-03-post-merge-reconciliation",
            "head_sha": "a" * 40,
            "merge_commit": "b" * 40,
            "reviews": [],
    ...
    reconciled_auth_text = authority_path.read_text(encoding="utf-8")
    reconciled_active_text = active_task_path.read_text(encoding="utf-8")
    assert "**Status**: MERGED" in reconciled_active_text
    ...
    monkeypatch.setattr(
        score2gp_governance_audit, "load_authority", lambda path: json.loads(reconciled_auth_text)
    )
    ...
    def mock_open(path, *args, **kwargs):
        from unittest.mock import mock_open as m_open
        if "ACTIVE_TASK.md" in str(path):
            return m_open(read_data=reconciled_active_text)()
        if "ORCHESTRATION_STATE.json" in str(path):
            return m_open(read_data=reconciled_auth_text)()
        if "AGENT-RULES.md" in str(path) or "AGENT_CONTROL.md" in str(path):
            return m_open(read_data="agent_verify.py artifact_audit.py pr_body.py")()
        return original_open(path, *args, **kwargs)
    ...
    def mock_run(args, **kwargs):
        if "gh" in args:
            raise AssertionError("gh should not be called for MERGED task")
        return subprocess.run(args, **kwargs)
