...
## 7. Governed Conversion-Recovery Programme

The four-task M6 implementation proposal is superseded as immediate execution
authority. It assumed disputed constants, contradictory measure counts, a
skip-as-pass private test strategy, and synthetic unit tests before the target
architecture and evidence contract had been reviewed.

The authoritative planning artifacts on this branch are:
...
Task 88 is the full evidence-adjudication and architecture-review task. It must
resolve the conflicting source reports and open PRs, design the real-source
test and private CI contract, decide the sidecar route, trace and redesign the
module seams, and specify the first unblocked downstream prompt. Publishing or
activating that prompt is a later, separate AgentOps governance change.

The previously drafted M6 implementation prompts are retained as blocked
skeletons. Their proposed constants and code edits are hypotheses, not accepted
architecture. They may be completed only after upstream evidence establishes
the correct seam, rule, negative oracle, and allowed files.

## 8. Testing and Acceptance Correction

Synthetic or mocked behavioural score data is banned as evidence for
recognition, grouping, timing, fingering, conversion, or GP correctness.
Behavioural tests must use complete real-world private fixtures or
provenance-linked cases extracted from them.

Private-corpus tests may be absent from public CI, but absence is
NOT_EVALUATED. A skipped private suite cannot satisfy review or merge
acceptance. Required evidence must include a non-skipped isolated run.

Pure format and schema tests may remain only when they make no recognition or
musical-fidelity claim. Refusal tests remain safety tests only and must be
paired with productive success evidence for supported inputs.

Before removing a synthetic false oracle, its real-source replacement must be
shown to fail on the exact known-bad branch or mutation. The generator process
must not receive or discover the reference GP path; comparison occurs
afterward in an independent oracle process.

Programme completion requires useful no-reference GP output with ordered
bar/event agreement, not a clean refusal, file creation, aggregate counts, or a
green synthetic suite.
