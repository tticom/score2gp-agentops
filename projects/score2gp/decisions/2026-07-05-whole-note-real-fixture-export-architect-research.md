...
Because the candidates lack a resolved pitch, the notation bridge (`build_ir_from_notation_outcomes`) filters them out:
```python
pitch_str = outcome.get("clef_resolved_staff_pitch")
if not pitch_str:
    continue
```
Since all note candidates are filtered out, the bridge raises `NotationBridgeInputError: no_valid_notation_outcomes_found`.
...
Even when passing `assume_treble_clef=True` directly into `run_recognition_on_file`, the pitch remains unresolved. This is because `map_assumed_treble_pitch_to_read_only_outcomes` explicitly filters out `whole_note_candidate`:
```python
def map_assumed_treble_pitch_to_read_only_outcomes(outcomes: list[dict]) -> None:
    ...
    for cand in outcomes:
        ...
        # ...
```
Whole notes were introduced in a later task (PR #334) and were never integrated into the fallback treble pitch mapping logic.

### Gap 3: CLI Does Not Expose or Pass `assume_treble_clef`
The `notation-whole-note-export` CLI command in `score2gp/cli.py` does not accept any option to assume a treble clef, nor does it pass `assume_treble_clef=True` when calling `run_recognition_on_file`.

### Gap 4: Test Suite Gaps (Mock-only CLI Tests)
The CLI test suite (`test_cli_notation_whole_note_export.py`) verifies the CLI by mock-patching the output of `run_recognition_on_file` to include `"clef_resolved_staff_pitch": "B4"`. This mock-patched check hides the E2E integration failures.

---

## 4. Next Required Loop Step

### Recommended Actions (to be authorised in a future Developer task)
...
2. **Update `notation-whole-note-export` CLI**: Accept an optional `--assume-treble-clef` flag (or make it default to True for this command, similar to how it is used) and forward it to `run_recognition_on_file`.
...
### Next Step
Proceed to **Reviewer architecture verification** to approve this research record and authorize the subsequent narrow Developer fix task.
