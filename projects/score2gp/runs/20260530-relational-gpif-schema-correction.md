...
- **Track ID Sequence Mapping (Crash Cause 4 - Immediate Crash)**: Discovered that copy-pasted companion files from original templates (such as `Content/ScoreViews/*.gpsv`, `PartConfiguration`, `Preferences.json`) contain hardcoded binary/XML references to track ID `"0"`. Using `ScoreIR` track ID `"gtr-1"` broke these references, causing the application to fail resolving track IDs and crash immediately on startup. We resolved this by mapping track IDs to sequential 0-indexed integers (`"0"`, `"1"`, etc.) in the relational compiler (`gpif.py`), cleanly restoring all references across binary/XML database companion files.
- **SystemsLayout Measure Partitioning (Crash Cause 5 - Page Layout Crash)**: Identified that writing a single number for `SystemsLayout` (e.g., `3`) while the score has 66 measures causes immediate layout engine crashes in Guitar Pro's page layout engine. We implemented a robust system partition generator that divides the total measures `M` into a space-separated layout list whose sum equals exactly `M` (e.g., `"3 3 ... 3"`), completely eliminating out-of-bounds rendering crashes.
- **Preserve Template Binary Companion Files (Crash Cause 6 - Immediate Crash)**: Identified a critical crash cause where `Content/LayoutConfiguration` and `Content/Preferences.json` are written as XML/JSON inside the `.gp` zip package, whereas native GP8/7 files require their exact native binary structures. Overwriting these files destroyed the template's binary structure, triggering immediate crashes on startup. We resolved this by changing `gp_package.py` to use `setdefault` for `VERSION`, `Preferences.json`, and `LayoutConfiguration`. When a template is provided, this cleanly preserves the pristine, native binary-compatible companion files from the template zip, completely eliminating immediate startup crashes!
- **Missing Explicit Directory Headers (Crash Cause 7 - Immediate Crash)**: Discovered that when building the `.gp` zip package, directory structures (such as `Content/ScoreViews/` and `Content/Stylesheets/`) were not explicitly defined as directory entries in the zip archive headers. While python's zipfile extraction automatically creates missing subfolders, strict C++/Qt zip extractors (such as the one native to the official Guitar Pro application) throw exceptions or crash if they attempt to write files into subdirectories that haven't been explicitly declared in the zip headers first. We resolved this by extracting and declaring all parent directories from the keys of the zip map dynamically in `gp_package.py` and writing them to the zip headers, completely eliminating platform-independent decompression crashes!
