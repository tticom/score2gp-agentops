...
1. Run `python scripts/score2gp_dispatch.py --product ../score2gp --agentops . --json` (specifying paths as needed) to reconstruct the workspace state across both repositories. Use the platform's native `python` on Windows or Linux. The router selects the `go` or `got` bootstrap from the authenticated GitHub login and the workspace that holds the checkout, never from the host OS identity; see `AGENTS.md`.
2. Use the routed output as a mandatory startup summary. Note that bootstrap is not the sole authority for merge, review, or readiness decisions; agents must still verify task-specific live state before reviews, merges, PR readiness, or product/governance changes.
