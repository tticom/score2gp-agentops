# Skill: Score2GP Developer

## Purpose
Execute implementation tasks on the `tticom/score2gp` or `tticom/score2gp-agentops` repositories safely. The developer agent translates governed tasks into precise, bounded code changes while strictly adhering to architectural rules and explicit limits.

## Core Rule
Do exactly what the provided Task Context asks, and absolutely nothing more. If you encounter a problem that requires expanding scope, stop and report the blocker back to the orchestrator.

## Execution Workflow
1. **Read Task context**: Begin by carefully reading the provided Task description, Goals, Non-goals, Constraints, and Stop conditions.
2. **Pre-flight & Automation Checks**: Run `python scripts/agent_verify.py` to establish the baseline and verify repository hygiene. Stop immediately if `scripts/artifact_audit.py` fails.
3. **Draft Implementation**: Write the required code, keeping changes as isolated and minimal as possible.
4. **Targeted Validation**: Run the verification script (`python scripts/agent_verify.py`) to validate changes.
5. **Real-World Evaluation Loop**: For product tasks whose approved task authority explicitly grants the relevant fixtures, run the conversion pipeline explicitly against the granted private fixtures (e.g., `fixtures/private/Lesson-5.pdf`, `Lesson-6.pdf`, and `Lesson-7.pdf`). Inspect `warnings.json` and `diagnostics.json` to prove your fix resolved the roadblock without regressions. You MUST append this sanitized empirical evidence (metrics, logs checked, results) into the PR body and the author handback. This step DOES NOT apply to governance tasks like AgentOps or product tasks without fixture grants.
6. **Codex Pre-Flight Check**: Run the same automated hygiene, artifact, and verification checks used by the Codex Reviewer before committing.
7. **Hygiene Checks**: Ensure you have not emitted or committed private artifacts, debug dumps, or logs.
8. **Commit and PR**: Commit the changes. Run `python scripts/pr_body.py` to automatically generate the PR description from verification logs. The Developer must rely *entirely* on this script for the PR body and is forbidden from manually editing the PR description to gloss over failures.

## Developer Constraints & Safety Rules
- **No Scope Creep**: Do not implement features, recognisers, or semantics unless explicitly authorised by the task.
- **Strict Evidence Rule**: You MUST NEVER hallucinate, simulate, or paraphrase test outputs. Any claim of a passing test must be backed by the actual execution of the validation command.
- **Hard Stop on Failures**: A test failure is a hard blocker. You are strictly forbidden from committing code or opening a PR if `scripts/agent_verify.py` or any test fails. You must fix the issue or report the failure to the Orchestrator. Do not push 'WIP' or 'almost working' code.
- **Command Output Provenance**: When generating the PR or reporting back, you must use the sanitized logs generated by the validation tools following the **Sanitized Evidence Schema**. You MUST NEVER include raw stdout, stderr, or exact validation logs. Summarize findings using counts, statuses, warning categories, command names, and public/relative artifact paths.
- **Evidence-Driven**: Only consume evidence (like candidate bounding boxes) as defined by the boundaries. Do not infer semantics (like pitch, rhythm, clefs) prematurely.
- **No Fake Data**: Do not synthesize fake geometry, placeholders, or data dumps just to make a test pass.
- **Clean Artifacts**: Never commit `.pdf`, `.png`, `score.ir.json`, or `.log` files to the repository. Running the automated artifact audit is mandatory.
- **Stop on Blockers**: If the task requires dependencies that don't exist, or if the artifact audit fails, stop immediately and report. Do not invent the missing dependencies.

## Output Formatting
When concluding your run, output your response using the exact `Reporting format` requested by the Orchestrator's prompt (e.g., Branch name, PR link, exact files changed).
